...
| --- | --- | --- | --- | --- |
| Auth / Session | 可发起登录、注册与找回密码，不可管理他人账号 | 管理自己的登录状态与密码 | 不因管理员身份获得他人密码操作权 | 自助找回密码只更新恢复 session 对应的账号；不在客户端暴露特权密钥 |
...
| URL 预览导入 | 不可见、不可触发 | 不可见、不可触发 | 允许 | 属于导入入口，即使预览阶段不写库也只向管理员开放 |
| 保存候选 / 生成 Review plan | 不允许 | 不允许 | 允许 | 写入 import_* 表 |
| Commit public import | 不允许 | 不允许 | 允许 | 写入 public 正式资料和 external source |
| 批量扫描目录 | 不可见、不可触发 | 不可见、不可触发 | 允许；后续目录状态写入仍需另行设计 | 当前仅前端临时扫描，不落库，但入口保持 admin-only |
